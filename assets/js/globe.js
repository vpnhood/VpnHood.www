...
var props = {
  ...
  startingCountry: 'germany', // The key of the country to rotate the camera to during the introduction animation (and which country to start the cycle at)
  colours: {
...
function getData() {
  ...
  request.onload = function() {
    if (request.status >= 200 && request.status < 400) {
      ...
      window.addEventListener('scroll', checkGlobeInView);
    }
    else {
      showFallback();
    }
...
function checkGlobeInView(){
  const globe = document.getElementById("globeWrapper");
  const rect = globe.getBoundingClientRect();
  const isOnScreen = rect.top < window.innerHeight && rect.bottom > 0;

  if (isOnScreen) {
    setupScene();
    window.removeEventListener('scroll', checkGlobeInView);
  }
}
...
function setupScene() {
  ...
  var canvasResizeBehaviour = function() {
    ...
    if (pageSize >= 992) {
        container.width = lgWidth+'px';
        container.height = lgHeight+'px';
    }
    else if (pageSize < 992 && pageSize >= 768) {
        container.width = mdWidth+'px';
        container.height = mdHeight+'px';
    }
    else if (pageSize < 768 && pageSize >= 576) {
        container.width = smWidth+'px';
        container.height = smHeight+'px';
    }
    else {
        container.width = xsmWidth+'px';
        container.height = xsmHeight+'px';
    }
