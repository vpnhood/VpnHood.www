...
- **Language selector — `_includes/lang-selector.html` + `_data/languages.yml` + `assets/js/lang.js`.** Renders **nothing until a second language exists**: languages are discovered from `site.data.i18n` folder keys (no registry — a language appears when vhtranslator drops its `_data/i18n/<code>/` folder); `_data/languages.yml` only maps codes to native display names. Used twice — `header.html` passes `style="dropdown"` (desktop `#topRightMenu`), `footer.html` passes `style="list"` (mobile offcanvas). Each entry links to the current page's counterpart when it exists (checked against `site.pages`), else that language's home. `lang.js` persists clicks in `localStorage vh_lang` and, **on English pages only** (crawler-safe), redirects a stored/browser-preferred language to its translation using the page's own hreflang alternates (never 404s). Preview with `?vh_lang=fa`.
...
## Translations (locale trees — in progress)
One command translates the whole site: **`vhtranslator site`** (VpnHood.Tools.ResourceTranslator; config at `vh_translator/vhtranslator.json`, currently `fa`). It does two things per language:

1. **Data**: translates the `_data/i18n/en/` language folder (config `"data": ["_data/i18n/en"]`) file-by-file, key-by-key (incremental) into sibling folders (`_data/i18n/fa/…`). Page bodies self-localize from these via `page.lang` — nothing else needed.
2. **Pages**: generates per-language page copies (`fa/free-vpn/index.html` → `/fa/free-vpn/`) with `lang:` + `auto_translated: true` front matter. The config sets **`"pageBody": "copy"`** — bodies are copied **byte-identically** (they're all `{{ t.* }}` variables); only front-matter `title`/`description` are translated (the `titleMustContain: "VpnHood!"` rule still applies).

**Never hand-edit generated pages or generated data folders** (overwritten/pruned by the tool; fixes go in `vh_translator/custom_prompt.txt`, per-key edits in the generated JSON survive until the source key changes, or delete the `auto_translated` line to own a page permanently). Opt a page out with `translate: false` front matter (or config `exclude` globs — legal pages are excluded); opt a fragment out with the standard `translate="no"` attribute. `header.html` derives **reciprocal hreflang + `x-default`** from the generated pages' `lang` front matter and sets `<html lang>`/`dir` from `page.lang` — see [.docs/seo-and-semantic-html.md](.docs/seo-and-semantic-html.md) §8 before touching any of it. Never add body-level `inLanguage` microdata. Translator bookkeeping (`site_watch.json`, `i18n_*_watch.json`) lives in `vh_translator/` — commit it.

**Language-aware links (`vh_base`)**: `header.html` assigns `vh_base` once per page (`''` on English, `/fa` on Persian, …; Jekyll includes share Liquid scope). **Every internal link must be written `href="{{ vh_base }}/free-vpn/download"`** so translated pages link within their own tree — a bare `href="/free-vpn"` on a fa page would jump back to English. Exceptions (never prefixed): legal pages (`/privacy-policy`, `/terms-of-use`, `/vpnhood-*-privacy-policy` — English only by policy), `/assets/...`, and external URLs. The header/footer/FAQ chrome strings are still hardcoded English (extraction pending).
