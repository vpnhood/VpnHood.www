...
- **Geo lookup** is same-origin-first with a `www.cloudflare.com` fallback (covers `www` being DNS-only). Order matters and must not flip — see [.docs/china-bar-geo.md](.docs/china-bar-geo.md) before changing the geo source or endpoint order.
