...
The site is machine-translated by **`vhtranslator site`** (VpnHood.Tools.ResourceTranslator; config at `vh_translator/vhtranslator.json`, currently `fr` only). Generated pages land in per-language trees mirroring source paths (`fr/free-vpn/index.html` → `/fr/free-vpn/`) with `lang:` + `auto_translated: true` front matter — **never hand-edit generated pages** (they are overwritten/pruned by the tool; fixes go in `vh_translator/custom_prompt.txt`, or delete the `auto_translated` line to own a page permanently). Opt a page out with `translate: false` front matter (or config `exclude` globs — legal pages are excluded); opt a fragment out with the standard `translate="no"` attribute. `header.html` derives **reciprocal hreflang + `x-default`** from the generated pages' `lang` front matter and sets `<html lang>` from `page.lang` — see [.docs/seo-and-semantic-html.md](.docs/seo-and-semantic-html.md) §8 before touching any of it. Never add body-level `inLanguage` microdata.
