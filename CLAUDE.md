...
- JS: framework JS (jQuery + the stock Bootstrap 5 bundle, plus Helix `main.js`) is loaded in the `<head>` from `assets/js/vendor/` / `assets/js/`. Bootstrap is the single stock `vendor/bootstrap.bundle.min.js` (Popper included) — the old Joomla-customized per-component ES modules and `core.min.js` have been removed (see Gotchas). VpnHood's own scripts (`main.js`, `globe.js`, `ThreeOrbitControls.js`, `vh-general.js`, `china-bar.js`) are in `assets/js/`.
...
- Bootstrap JS is the **stock 5.3.x bundle** (`assets/js/vendor/bootstrap.bundle.min.js`, Popper included) — loaded `defer` after jQuery, before `main.js`. It replaced the old Joomla-customized per-component ES modules (which hard-referenced the `Joomla` global from `core.min.js`); both that `bootstrap/` dir and `core.min.js` are gone. The only Bootstrap-JS feature the site actually uses is the FAQ accordion (collapse data-API); everything else is CSS/jQuery.
