...
- **Pushing `main` deploys production.** `.github/workflows/jekyll.yml` builds with the pinned Jekyll and publishes the generated `_site` to GitHub Pages (`actions/deploy-pages`); served at https://www.vpnhood.com **behind Cloudflare**. Triggers: `push` (publish), `workflow_dispatch` (manual), and a weekly `schedule` cron (legal sync — see below). Never switch Pages back to the legacy branch builder (it runs an old Jekyll).
- Custom domain via the `CNAME` file (`www.vpnhood.com`) so `baseurl` stays `""` and the site's absolute `/assets/...` paths resolve. The domain **must stay proxied through Cloudflare (orange cloud)** — several features depend on it (geo, http→https). Cloudflare SSL/TLS mode must be **Full** (Flexible causes a redirect loop with GitHub Pages).
...
- JS: framework JS (jQuery, Joomla core, Helix `main.js`, Bootstrap ES modules) is loaded in the `<head>` from `assets/js/vendor/`. **The Bootstrap modules under `assets/js/vendor/bootstrap/` must stay together** — they `import` sibling `./dom.min.js` / `./popper.min.js`. VpnHood's own scripts (`main.js`, `globe.js`, `ThreeOrbitControls.js`, `vh-general.js`, `china-bar.js`) are in `assets/js/`.
...
- **Geo lookup is same-origin-first, with a `www.cloudflare.com` fallback.** `detect()` fetches same-origin `/cdn-cgi/trace` first; if that returns no `loc=` (e.g. the `www` record is DNS-only/grey-cloud so the request hits GitHub Pages and 404s), it falls back to `https://www.cloudflare.com/cdn-cgi/trace` — always Cloudflare-proxied and CORS-enabled (`Access-Control-Allow-Origin: *`), so the cross-origin read is allowed. **Same-origin must stay the primary**, not the other way around: `www.cloudflare.com` is frequently throttled/blocked inside mainland China, so a CN visitor who *can* reach our proxied site might *not* reach `cloudflare.com` — flipping the order would lose detection for the exact audience the bar targets. The fallback exists only to cover `www` not being proxied; it adds an extra DNS+TLS to a third-party origin, so it's the backstop, not the default. Both endpoints read the visitor IP at the anycast edge → identical `loc=`, so there's no accuracy difference. **The real fix for a `www` 404 is to orange-cloud the `www` DNS record** (the site also needs the proxy for http→https); the fallback just stops the bar from being collateral damage when it isn't.
...
- The Bootstrap ES modules need their `dom.min.js` / `popper.min.js` siblings present in `assets/js/vendor/bootstrap/`.
